...
- (void)setupSegmentButtons {
    ...
    if (_buttonText) {
        _buttonText = [[NSArray alloc] initWithObjects:@"first",@"second",@"third",@"fourth",@"etc",@"etc",@"etc",@"etc", nil];
    }
    
    _navigationView = [[UIView alloc] initWithFrame:CGRectMake(X_BUFFER, 0, self.view.frame.size.width - 2 * X_BUFFER, self.navigationBar.frame.size.height)];
    
    //buttons
    CGRect frameTemp = _navigationView.bounds;
    ...
    [_navigationView addSubview:_buttonContainer];
    //pageControl
    ...
    _pageControl = ({
        ...
        pageControl.frame = (CGRect){0, SELECTOR_Y_BUFFER, CGRectGetWidth(_navigationView.frame), SELECTOR_HEIGHT};
    ...
    [_navigationView addSubview:_pageControl];
...
- (void)setupPageViewController {
    if ([self.topViewController isKindOfClass:[UIPageViewController class]]) {
        ...
        [_pageController setViewControllers:@[_viewControllerArray[0]] direction:UIPageViewControllerNavigationDirectionForward animated:YES completion:nil];
        [self syncScrollView];
    }
}
    ...
    if (!self.isPageScrollingFlag) {
        ...
        if (button.tag > tempIndex) {
            ...
            for (int i = (int)tempIndex + 1; i <= button.tag; i++) {
                [_pageController setViewControllers:@[_viewControllerArray[i]] direction:UIPageViewControllerNavigationDirectionForward animated:YES completion:^(BOOL finished) {
                    if (finished) {
        ...
        //%%% this is the same thing but for going right -> left
            ...
            for (int i = (int)tempIndex - 1; i >= button.tag; i--) {
                [_pageController setViewControllers:@[_viewControllerArray[i]] direction:UIPageViewControllerNavigationDirectionReverse animated:YES completion:^(BOOL finished) {
                    if (finished) {
                        [weakSelf updateCurrentPageIndex:i];
                    }
                }];
...
- (void)setCurrentPageIndex:(NSInteger)currentPageIndex {
    ...
    [self.viewControllerArray enumerateObjectsUsingBlock:^(UIViewController *obj, NSUInteger idx, BOOL * _Nonnull stop) {
